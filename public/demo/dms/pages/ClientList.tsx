...
const ClientList: React.FC = () => {
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-12">
      ...
      <div className="bg-white p-3 md:p-4 rounded-[12px] shadow-sm border border-gray-100 flex flex-col md:flex-row gap-3 md:gap-4 md:items-center">
        <div className="relative flex-1">
          <SearchIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
          <input
            type="text"
            placeholder="会社名、業種、担当者で検索..."
            className="w-full pl-11 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#c9a962]/50 focus:bg-white transition-all text-sm md:text-base"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
        </div>
        <div className="flex flex-wrap gap-2 md:gap-4">
          <select className="flex-1 md:flex-none px-3 md:px-4 py-2.5 md:py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#c9a962]/50 text-sm md:text-base">
