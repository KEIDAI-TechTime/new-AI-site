  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-serif font-bold text-[#0a1628] mb-2">顧問先一覧</h1>
        ...
        <div className="flex gap-2 md:gap-3">
          <Button variant="secondary" icon={<FileUp size={16} className="md:w-[18px] md:h-[18px]" />}>
            <span className="hidden sm:inline">書類一括登録</span><span className="sm:hidden">一括登録</span>
          </Button>
          <Button icon={<Plus size={16} className="md:w-[18px] md:h-[18px]" />}>
            <span className="hidden sm:inline">顧問先を追加</span><span className="sm:hidden">追加</span>
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
        {stats.map((stat, idx) => (
          <div key={idx} className="bg-white p-4 md:p-6 rounded-[12px] shadow-sm border border-gray-100 flex items-center justify-between">
            <div>
              <p className="text-xs md:text-sm text-gray-400 font-medium mb-1">{stat.label}</p>
              <p className="text-2xl md:text-3xl font-serif font-bold text-[#0a1628]">{stat.value}</p>
            </div>
            <div className="bg-[#f8f6f1] p-3 md:p-4 rounded-xl">
              {stat.icon}
            </div>
          </div>
        ))}
      ...
      <div className="bg-white p-3 md:p-4 rounded-[12px] shadow-sm border border-gray-100 flex flex-col lg:flex-row gap-3 md:gap-4">
        ...
        <div className="flex gap-2 md:gap-4 overflow-x-auto">
          <select className="px-3 md:px-4 py-2.5 md:py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#c9a962]/50 text-sm md:text-base flex-shrink-0">
          ...
          <div className="md:hidden flex-shrink-0">
            <Button variant="secondary" icon={<Filter size={16} />}><span className="sr-only">フィルタ</span></Button>
          </div>
        </div>
      </div>
